...
namespace DHTCrawler
{
    ...
    class DHTBucketManager
    {
        ...
        /// <summary>
        /// Initializes lDHTNodes with around 10 million nodes, the list is loaded from disk if available
        /// </summary>
        ...
        public DHTBucketManager(IPEndPoint bootstrapAdr, int localport)
        {
            ...
            int cycleCount = 1;
            while (nofNodes < 5000000)
            {
                for (int i = 0; i < Program.DHT_BUCKETS && nofNodes < 5000000; i++)
                {
                    Console.Write("\r{0}%   ", nofNodes / 50000);
                    ...
                    if(getPeerReqSent > cycleCount*500000)
                    {
        ...
        private void SendMessage(IPEndPoint destination, string message)
        {
            ...
            try
            {
                ...
                //s.SendTo(send_buffer, destination);
                //Log(message);
            }
            catch (Exception ex)
            {
